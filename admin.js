...
document.getElementById('checkPaystackBtn').onclick = async () => {
  ...
  try {
    const response = await fetch('/api/admin/status/paystack');
    if (response.status === 401) {
      el.textContent = '✗ You have been logged out - refresh the page and log in again.';
      el.style.color = '#c0392b';
      return;
    }
    const res = await response.json();
    el.textContent = (res.connected ? '✓ ' : '✗ ') + (res.message || 'Something went wrong checking this - try again.');
    el.style.color = res.connected ? '#2a7a2a' : '#c0392b';
  } catch (err) {
    el.textContent = '✗ Could not reach the server. Try refreshing the page.';
    el.style.color = '#c0392b';
  }
};
...
document.getElementById('checkPrintifyBtn').onclick = async () => {
  ...
  el.textContent = 'Checking...';
  try {
    const response = await fetch('/api/admin/status/printify');
    if (response.status === 401) {
      el.textContent = '✗ You have been logged out - refresh the page and log in again.';
      el.style.color = '#c0392b';
      return;

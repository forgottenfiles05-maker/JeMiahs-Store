...
// shipping/contact info, then hand off to Paystack for the actual payment.
const checkoutOverlay = document.getElementById("checkoutOverlay");
document.getElementById("checkoutBtn").onclick = async () => {
  if (cart.length === 0) { alert("Your bag is empty."); return; }

  const email = prompt("Email for your order:");
  if (!email) return;
...
document.getElementById("closeCheckout").onclick = () => checkoutOverlay.classList.remove("open");
